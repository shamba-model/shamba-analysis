"""Run the OAT (one-at-a-time) sensitivity batch locally, one site at a time.

Sites are fully independent, and each site's own OAT run already fans its
evaluations across every core via runner.py's ProcessPoolExecutor.
Run sites serially here.

For each site, this script:
  1. Stages projects/oat_batch_XXXX/input/ (climate, soil-info, common
     inputs, and the {}_* templates renamed to site_XXXX_*).
  2. Runs run_oat_direct.py as a subprocess (each site has its own process,
...
When the batch finishes it collapses each site's oat_results.csv down to the
year="total" rows (one row per parameter x direction, i.e. baseline plus each
parameter's min/max) and writes one row per (site_index, parameter, direction)
to oat_results_all_sites.csv. Pass --aggregate-only to rebuild that table from
existing results without rerunning.

Usage (inside Docker; host shamba/ is mounted as /app, so no "shamba/" prefix):
    poetry run python /app/model/morris/run_oat_batch.py
    poetry run python /app/model/morris/run_oat_batch.py --start 0 --end 49
    poetry run python /app/model/morris/run_oat_batch.py --aggregate-only

NB: runs faster if it is writing files that don't already exist, rather than
...
RUN_OAT_DIRECT = SHAMBA_DIR / "run_oat_direct.py"

# Common input files copied verbatim into every site's input folder. The three
# species-lookup tables are read from INPUT_DIR by run_oat_direct.py, so they
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--n-proj-cohorts", type=int, default=1,
                   help="Passed to run_oat_direct.py (default: 1)")
    p.add_argument("--n-base-cohorts", type=int, default=1,
                   help="Passed to run_oat_direct.py (default: 1)")
    p.add_argument("--results-dir", default=str(MORRIS_DIR / "results" / "oat_batch"),
                   help="Where collected per-site outputs are copied")
    p.add_argument("--resume", action="store_true",
                   help="Skip sites whose collected oat_results CSV already exists")
    p.add_argument("--stop-on-error", action="store_true",
                   help="Abort the batch on the first site that fails (default: keep going)")
    p.add_argument("--aggregate-only", action="store_true",
                   help="Skip the batch; just rebuild oat_results_all_sites.csv from existing results")
    p.add_argument("--no-aggregate", action="store_true",
                   help="Don't build the combined oat_results_all_sites.csv after the batch")
...
def aggregate_results(results_dir: Path) -> Path | None:
    """
    Reads results_dir/oat_results_oat_batch_XXXX.csv (each with columns
    parameter,direction,year,emissions_diff,effect_vs_baseline), takes the
    year="total" rows per parameter/direction (baseline, plus each
    parameter's min/max run, summed over the whole project horizon), and
    writes one row per (site_index, project_name, parameter, direction) to
    results_dir/oat_results_all_sites.csv. Returns the output path, or None
    if no per-site files were found.
    """
    file_prefix = "oat_results_oat_batch_"
    out_path = results_dir / "oat_results_all_sites.csv"
    ...
    for path in result_files:
        project_name = path.stem[len("oat_results_"):]   # oat_batch_0007
        site_index = int(path.stem[len(file_prefix):])   # 7
        df = pd.read_csv(path)
        totals = df[df["year"] == "total"].reset_index(drop=True)
...
def collect_outputs(project_name: str, results_dir: Path) -> bool:
    ...
    wanted = ("oat_results.csv", "oat_design_X.npy", "oat_outputs_Y.npy")
    all_found = True
    for name in wanted:
        src_path = src / name
...
def collected_results_path(project_name: str, results_dir: Path) -> Path:
    return results_dir / f"oat_results_{project_name}.csv"
...
def run_site(project_name: str, prefix: str, args: argparse.Namespace,
    ...
    cmd = [
        sys.executable, str(RUN_OAT_DIRECT),
...
def main() -> None:
    ...
    soil_info = read_testsites(testsites_path)
    end = args.end if args.end is not None else len(soil_info) - 1
    if not (0 <= args.start <= end < len(soil_info)):
        raise ValueError(
            f"site range {args.start}..{end} is out of bounds for "
            f"{len(soil_info)} sites (valid indices 0..{len(soil_info) - 1})"
        )
    ...
    total = len(indices)
    print(f"OAT batch: {total} sites ({args.start}..{end}), serial.")
    print(f"  bounds file : {bounds_path}")
    print(f"  results dir : {results_dir}")
    print(f"  per-site log: {PROJECTS_DIR / 'oat_batch_XXXX' / 'oat_run.log'}")
    print()
    ...
    for site_index in indices:
        ...
        if not args.dry_run and log_path.exists():
            shutil.copyfile(log_path, log_files_dir / f"oat_run_{project_name}.log")
    ...
    if n_failed:
        print("Some sites failed; check their oat_run.log and the batch_log.csv status column.")
        sys.exit(1)
